...
public interface IFalsifier
{
    ...
    /// <summary>
    /// Mutates <paramref name="scope"/> and re-runs the repo's own existing test suite (no LLM,
    /// no synthesized candidate) against every mutant: "do your existing tests kill mutants?"
    /// </summary>
    /// <param name="testProjectPath">Path to the existing test project's .csproj.</param>
    /// <param name="scope">Which files may be mutated, and the mutant ceiling.</param>
    /// <param name="cancellationToken">Token to cancel the mutation run.</param>
    /// <exception cref="AttestMutantCountMismatchException">
    /// The mutator tested a mutant outside the requested scope.
    ...
    /// The mutator tested more mutants than <see cref="MutationScope.MaxMutants"/> allows.
    /// </exception>
    /// <exception cref="AttestCompareSuiteFailedException">The run produced no mutation report to read.</exception>
    Task<CompareSuiteResult> CompareSuiteAsync(string testProjectPath, MutationScope scope, CancellationToken cancellationToken);
}
