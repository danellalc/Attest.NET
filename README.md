...
|---|---|---|---|
| `Attest.DotNet` | FsCheck | Stryker.NET | **v1, in development** |
...
*Note on naming: an unrelated `Attest.*` package family (a fakes framework) exists on NuGet, and an unrelated `attest-framework` exists on GitHub. This project is `Attest.DotNet` / `Attest.Cli`, at `github.com/danellalc/Attest.DotNet` — the repo name matches the package on purpose, so the link itself disambiguates.*
