...
**v1.x: quick wins**
**`--compare-suite` (built):** runs the Falsifier against the tests the repo already has: "do your existing tests kill mutants?" answered with a number, no LLM involved -- `attest --compare-suite --diff <base-ref> --test-project <path>`. Close to a standalone entry product and the strongest possible demo material, so it shipped first among the v1.x items. **SARIF and JSON output formats (built):** `--format json|sarif` -- JSON for downstream tooling, SARIF for GitHub code scanning (one `note`-level result per delivered property, anchored at the killed mutant's location; rejected/quarantined candidates have no source location to anchor a SARIF result at). **`--trace-id` (built):** pass-through requirement/ticket tag attached to every delivered property in `--format json` output, no new verification. **Signed evidence export (built):** `--export-evidence <path>` writes a compact-JSON report plus a `<path>.sha256` digest -- HMAC-SHA256 (a real signature) if `ATTEST_EVIDENCE_KEY` is set, plain SHA-256 (tamper-evident, not authorship-proving) otherwise. **`--max-llm-cost` (built):** a circuit breaker, not a pre-call budget check -- the one proposal call already happened by the time cost is known, so this refuses to *deliver* an over-budget report rather than preventing the spend. Still roadmap: PR comments that edit in place instead of stacking (the #1 complaint against existing PR bots); webhook notifications (Slack/Teams-compatible, a single POST to a URL the user owns, no account, no hosting).
